...
// Removed application.h dependency for cross-platform compatibility
#include "../math/math.h"
...
public:
	...
	// Get current framebuffer color buffer (for displaying with external UI)
	RGBA* getColorBuffer() const { return mFrameBuffer ? mFrameBuffer->mColorBuffer : nullptr; }
	uint32_t getFrameBufferWidth() const { return mFrameBuffer ? mFrameBuffer->mWidth : 0; }
	uint32_t getFrameBufferHeight() const { return mFrameBuffer ? mFrameBuffer->mHeight : 0; }
