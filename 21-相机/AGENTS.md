...
当前目录 `21-相机` 对应课程第 21 节：**相机（Camera）**。`main.cpp` 中演示了使用 `Camera` 类管理视图矩阵与投影矩阵，通过 `lookAt` 矩阵将相机位置、朝向和目标点转换为视图变换，并配合 `TextureShader` 渲染一个带纹理的三角形。
...
├── main.cpp                    # 程序入口（标准 main）。初始化 GPU + Slint UI，准备 VAO/VBO/EBO/Shader/Camera，通过 slint::Timer 驱动渲染
...
├── application/
│   ├── CMakeLists.txt          # 编译为静态库 applicationLib（image + camera 模块）
│   ├── camera.h / .cpp         # Camera 类：管理相机位置、朝向、视图矩阵与投影矩阵，支持 lookAt
...
│   │   ├── textureShader.h     # 纹理着色器声明：支持 MVP 矩阵与 diffuse 纹理采样
│   │   └── textureShader.cpp   # 纹理着色器实现
...
│   └── mathFunctions.h         # 数学工具：lerp、dot、cross、length、normalize、scale/translate/rotate/perspective/orthographic/screenMatrix/lookAt
├── assets/
...
`Camera` 类封装了相机系统的核心逻辑：

- **位置（Position）**：相机在 world space 中的坐标
- **朝向（Front）**：相机看向的方向，由 `yaw` 和 `pitch` 欧拉角计算得到
- **上向量（Top）**：相机的上方向，默认 `(0, 1, 0)`
- **视图矩阵（View Matrix）**：每帧通过 `math::lookAt(mPosition, mPosition + mFront, mTop)` 计算
- **投影矩阵（Projection Matrix）**：在构造时通过 `math::perspective` 创建

### 输入处理（当前版本）

由于本版本使用 Slint UI 框架，而当前 Slint C++ API 未直接暴露键盘/鼠标事件回调，因此 `Camera` 的交互输入接口（`onKeyDown` / `onKeyUp` / `onRMouseDown` / `onMouseMove` 等）在 `main.cpp` 中暂未连接。相机以固定位置和朝向渲染场景，主要用于演示 `lookAt` 矩阵与 `Camera` 类的结构。

若需恢复交互控制，可通过以下方式扩展：
1. 在 `.slint` 中使用 `TouchArea` 捕获指针事件并通过 callback 传回 C++
2. 或升级 Slint SDK 到支持 `on_key_pressed` / `on_pointer_moved` 的版本
...
| `application` | 图像加载 + 相机管理 | `Image`（基于 stb_image 加载图片）、`Camera`（视图/投影矩阵） |
| `gpu` | 渲染核心 | `GPU`、`FrameBuffer`、`BufferObject`、`VAO`、`Shader`、`Raster`、`Clipper`、`Texture` |
| `gpu/shader` | 着色器实现 | `DefaultShader`（MVP + 颜色传递）、`TextureShader`（MVP + diffuse 纹理） |
...
验证渲染结果的主要方式是：
- **目视检查**：运行 `softRenderer`，观察窗口中三角形的颜色、深度顺序、纹理映射是否正确。
- **对比预期**：课程每一节通常有明确的学习目标（如本节为 Camera + lookAt），通过观察三角形是否正确定位来验证管线行为。
...
4. **无多线程**：渲染管线、Slint 事件循环均为单线程，不存在并发安全问题。
5. **编码问题**：源代码文件中的中文注释使用 GBK / GB2312 编码存储。在 UTF-8 默认环境下可能显示为乱码，建议用支持 GBK 的编辑器查看，或统一转换为 UTF-8（但不影响编译）。
