...
void transform() {
  ...
  // 模型变换 - 绕Y轴旋转
  modelMatrix =
      math::rotate(math::mat4f(1.0f), angle, math::vec3f{0.0f, 1.0f, 0.0f});

  // 模拟摄像机往后退
  auto cameraModelMatrix =
      math::translate(math::mat4f(1.0f), math::vec3f{0.0f, 0.0f, cameraPos});
...
void prepare() {
  ...
  // 设置三个顶点的属性（颜色和纹理坐标）
  p1.color = RGBA(255, 0, 0, 255); // 红色
  p1.uv = math::vec2f(0.0f, 0.0f);

  p2.color = RGBA(0, 255, 0, 255); // 绿色
  p2.uv = math::vec2f(1.0f, 1.0f);

  p3.color = RGBA(0, 0, 255, 255); // 蓝色
  p3.uv = math::vec2f(1.0f, 0.0f);

  // 设置三个顶点的3D位置（齐次坐标）
  // 这些位置构成一个三角形，使用齐次坐标表示（w=1.0 表示这是一个点）
  pos1 = {-1.5f, 0.0f, 0.0f, 1.0f};
  pos2 = {1.5f, 0.0f, 0.0f, 1.0f};
  pos3 = {0.0f, 2.0f, 0.0f, 1.0f};

  // 创建投影矩阵: 用于将3D场景投影到2D视口上，创建透视效果
  perspectiveMatrix =
      math::perspective(60.0f, (float)WIDTH / (float)HEIGHT, 0.1f, 100.0f);

  // 创建屏幕变换矩阵: 将标准化设备坐标(NDC)转换为屏幕像素坐标
...
int main(int argc, char *argv[]) {
  ...
  mainWindow->on_close_requested([&app]() { app.quit(); });

  // 创建渲染定时器，约60fps(刷新率)
  slint::Timer renderTimer(
      std::chrono::milliseconds(16), [&mainWindow, &app]() {
        if (!app.isAlive())
          return;

        // 渲染一帧： 光栅化并将结果写入到GPU帧缓存区
        render();
        ...
        for (int y = 0; y < height; ++y) {
          ...
          for (int x = 0; x < width; ++x) {
            ...
            dest[destPos] =
                slint::Rgba8Pixel{pixel.mR, pixel.mG, pixel.mB, pixel.mA};
          }
        }
